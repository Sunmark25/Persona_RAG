...
def format_personality_dimensions(dimensions):
    formatted = []
    for dimension, traits in dimensions.items():
        formatted.append(f"{dimension}: {', '.join(traits)}")
    return "\n".join(formatted)
...
async def extract_entities(
    ...
    context_base = dict(
        ...
        examples=examples,
        language=language,
        target_character=PROMPTS["TARGET_CHARACTER"],
        personality_dimensions=format_personality_dimensions(PROMPTS["PERSONALITY_DIMENSIONS"])
    )
...
async def kg_query(
    ...
    sys_prompt = sys_prompt_temp.format(
        ...
        target_character=PROMPTS["TARGET_CHARACTER"],
        tuple_delimiter=PROMPTS["DEFAULT_TUPLE_DELIMITER"],
        personality_dimensions=PROMPTS["PERSONALITY_DIMENSIONS"],
    )
